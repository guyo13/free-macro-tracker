...
let foodObj2 = {
  ...
  nutritionalValue: {
    ...
    fats: 12,
    additionalNutrients: {
      polysat: { unit: "g", mass: 1.2 },
      fiber: { unit: "mg", mass: 450 },
    },
  },
}; // OK with Exceptions
let foodObj3 = {
  ...
  nutritionalValue: {
    ...
    fats: 12,
    additionalNutrients: {
      polysat: { units: "g", mass: 1.2 },
      fiber: { units: "mg", mass: 450 },
    },
  },
}; // OK
let foodObj4 = {
...
FMTReadAllNutrients(function (e) {
  let res = e.target.result.filter(function (entry) {
    if (entry.category === "Minerals") return true;
  });
  console.log(res);
});
...
function mealEntryAddTest(mepd, pidstart, pidstop, dstart, dstop, direction) {
  ...
  let nutritionalValues = [
    { calories: 60, proteins: 12, carbohydrates: 3, fats: 5 },
    {
      calories: 600,
      proteins: 33,
      carbohydrates: 22,
    ...
    },
    {
      ...
      additionalNutrients: {
        Minerals: [
          { name: "Calcium", mass: 10, unit: "mg" },
          { name: "Chloride", mass: 15, unit: "mg" },
        ],
        "Amino Acids": [
          { name: "Alanine", mass: 1500, unit: "mg" },
          { name: "Arginine", mass: 2000, unit: "mg" },
        ],
      },
    },
    {
  ...
  request.onsuccess = function (event) {
    ...
    if (cursor) {
      ...
      console.log(
        `Meal Entry count - ${count} == ${
          (pidstop - pidstart) * (dstop - dstart) * mepd
        } ? ${
          count == (pidstop - pidstart) * (dstop - dstart) * mepd
        }. Run time - ${(endTime - startTime) / 1000} Seconds.`
      );
      navigator.storage.estimate().then((e) => {
        console.log(
          `Quota: ${e.quota / 1000000} MB, Usage: ${e.usage / 1000000} MB`
        );
      });
    }
  };
...
FMTDataToStructuredJSON(function (records) {
  FMTExportToJSONBlob(records, function () {
    downloadData("FmtExport_Structured.json");
  });
});
FMTDataToJSONArray(function (records) {
  FMTExportToJSONBlob(records, function () {
    downloadData("FmtExport_array.json");
  });
});
setInterval(() => {
  document.getElementById("overview-date-next-day-lg").click();
}, 50);
...
function testSumNutiValAndRecipe() {
  ...
  FMTReadAllFoods(function (e) {
    ...
    FMTAddRecipe(recipeObj, function (e) {
